...
class MessageEntry extends ActiveRecord
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($insert) {
            /** @var User $orginator */
            $orginator = $this->getCreatedBy()->one();
            /** @var UserMessage[] $userMessages */
            $userMessages = UserMessage::find()
                ->where(['message_id' => $this->message_id])
                ->all();
            $mapToUser = function (UserMessage $val) {
                Yii::error("UserMsg: {$val->message_id} {$val->user_id}");
                return $val->getUser()->one();
            };
            $users = array_map($mapToUser, $userMessages);

            MailNotification::instance()
                ->from($orginator)
                ->about($this)
                ->sendBulk($users);
        }
